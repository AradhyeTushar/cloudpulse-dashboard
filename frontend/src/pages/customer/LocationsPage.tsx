...
import { Globe, Activity, CheckCircle, Search, Filter, Zap } from 'lucide-react';
...
export const LocationsPage: React.FC = () => {
  ...
  const filteredLocations = locations.filter((loc) => {
    const matchesSearch = loc.country.toLowerCase().includes(searchTerm.toLowerCase()) || loc.countryCode.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      >
        ...
        <div style={{ position: 'relative', minWidth: '280px' }}>
          ...
          <input
            ...
            placeholder="Search country or code (US, DE, Japan)..."
            value={searchTerm}
        ...
        {filteredLocations.map((loc) => (
          <div key={loc.id} className="card" style={{ padding: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              ...
              <span
                style={{
                  ...
                  fontSize: '0.7rem',
                  fontWeight: 700,
                  color: loc.status === 'optimal' ? '#10b981' : '#f59e0b',
                  background: loc.status === 'optimal' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(245, 158, 11, 0.1)',
                  padding: '0.2rem 0.5rem',
                  borderRadius: 'var(--radius-full)',
                  textTransform: 'uppercase',
                }}
              >
                <span style={{ width: 5, height: 5, borderRadius: '50%', background: loc.status === 'optimal' ? '#10b981' : '#f59e0b' }} />
                {loc.status}
              </span>
            </div>
            ...
            <div style={{ marginTop: '0.85rem', fontSize: '0.75rem', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
              ...
              <span>{loc.activeNodes.toLocaleString()} gateway edge nodes active</span>
            </div>
          </div>
        ))}
      </div>
