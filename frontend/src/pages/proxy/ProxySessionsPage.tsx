...
import { Radio, RefreshCw, XCircle, ShieldCheck, Zap } from 'lucide-react';
...
export const ProxySessionsPage: React.FC = () => {
  ...
  const handleTerminate = (sessionId: string) => {
    ...
    setSessions(proxyService.getStickySessions());
    showToast('Session Disconnected', `Session ${sessionId} has been terminated.`, 'info');
  };
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.75rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '0.35rem' }}>
            Monitor active persistent tunnels, view assigned exit IPs, and trigger on-demand IP rotation.
          </p>
        </div>
              ...
              {sessions.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                    No active sticky sessions right now. Connect to a sticky endpoint to spawn one.
                  </td>
                </tr>
              ) : (
                sessions.map((sess) => (
                  <tr key={sess.id} style={{ borderBottom: '1px solid var(--bg-border)' }}>
                    ...
                    <td style={{ padding: '1rem 1rem' }}>
                      <ProxyStatusBadge status={sess.status} />
                    </td>
